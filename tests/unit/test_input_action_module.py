...
# Test cases for ensuring None values are excluded from request parameters
@pytest.mark.parametrize("action_module_class, input_args, expected_params", [
    (GetJobs, {"status": "running", "name": None}, {
        "equals[status]": "running",
        "include": "name,status"
    }),
    (GetTasks, {"status": None, "name": "greg"}, {
        "equals[name]": "greg",
        "include": "name,status"
    }),
])
@patch("ansible_collections.itential.core.plugins.module_utils.http.send_request")
def test_action_module_exclude_none_values(mock_http_request, mock_http_login_response, mock_task_vars, action_module_class, input_args, expected_params):
    """Test action modules to ensure parameters with None values are excluded from the request."""
    ...
    second_call = mock_http_request.call_args_list[1]
    actual_params = second_call[1]["params"]

    # Ensure None values are excluded from the request
    ...
    for key in actual_params:
        assert key in expected_params or key == "token"
